view: fct_order_items {
  ...
  dimension: sk_created_date { type: string
    ...
    description: "Extracts the date part from the order creation timestamp."
  }

  dimension: sk_shipped_date { type: string
    hidden: yes
    sql: CASE WHEN ${sk_shipped_at_raw} IS NULL THEN NULL ELSE CAST(${sk_shipped_at_raw} AS DATE) END ;;
    description: "Date when the order item was shipped, or NULL if not yet shipped."
  }

  dimension: sk_delivered_date { type: string
    hidden: yes
    sql: DATE(${sk_delivered_at_raw}) ;;
    description: "Extracts the delivered date from the delivered_at timestamp. Returns NULL if delivered_at is blank."
  }
  ...
  dimension: sk_returned_date { type: string
    ...
    description: "The date when the order item was returned, extracted from the returned_at timestamp."
  }
